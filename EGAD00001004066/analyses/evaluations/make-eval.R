...
## Output directory
main.outdir <- "../../evaluations2/somatic_vcf"
...
prepare.eval.data <- function(ffpe_tumoral, sample_index) {
	...
	gatk_obmm_output <- read.delim(glue("{vcf.dir}/{sample_name}/{sample_name}.vcf.gz"), comment.char = "#", header = FALSE)
	### Select the necessary columns
	gatk_obmm_output <- gatk_obmm_output[, c("V1", "V2", "V4", "V5", "V7")]
	### Rename columns assigning them with appropriate names
	colnames(gatk_obmm_output) <- c("chrom", "pos", "ref", "alt", "filter")
...
for (index in seq_along(ffpe_tumoral)){
	...
	precrec_eval_metrics <- get.precrec.eval.metrics(eval_data_df, score_columns = c("FOBP", "VAFF", "SOB", "obmm"), model_names = c("mobsnvf", "vafsnvf", "sobdetector", "gatk-obmm"))
	...
	aucs <- get.multimodel.auroc.auprc(custom_multimodel_roc, custom_multimodel_prc)
