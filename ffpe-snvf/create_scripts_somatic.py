...
filtered_outdir = os.path.abspath("somatic_vcf")

models = ["mobsnvf", "vafsnvf", "sobdetector", "ideafix"]

outdir = "batch_scripts"
os.makedirs(outdir, exist_ok=True)
outdir = return_path_if_exists(outdir, abs=True)
...
for vcf_path in vcf_paths:
    ...
    for model in models:
        ...
        if model == "ideafix":
            ...
            content = f"#!/usr/bin/env Rscript\nRscript {template_dir} --vcf '{vcf_path}' --ref '{ref_path}' --outdir '{filtered_outdir}' '{ref_path}'\n"
            ...
            content = f"#!/bin/bash\nbash {template_dir} {bam_path} '../{vcf_path}' '../{filtered_outdir}'\n"
            filename = f"{outdir}/{model}_{sample_name}.sh"
