...
def filter_dataset(dataset: str, source_variant_set: str, target_variant_set: str) -> None:
	...
	for path in tqdm(ffpe_snvf_paths):
		...
		target_vars_path = f"{vcf_dir}/{sample_name}/{sample_name}.vcf.gz"
		target_vars = read_variants(target_vars_path)
		
		filtered_snvf = snvf.join(target_vars, on = ["chrom", "pos", "ref", "alt"], how="semi")

		filtered_snvf_outdir = f"{repo_root}/ffpe-snvf/{dataset}/{target_variant_set}/{model}/{sample_name}"
	...
	pl.DataFrame(filtering_summary).write_csv(f"{dataset}/{target_variant_set}/non-exome-exclusion_filtering-summary.tsv", separator="\t")
...
filter_dataset(
    ...
    source_variant_set = "somatic_filtered",
    target_variant_set = "somatic_filtered-exome"
)
